...
const fetchImages = async function ({trashpoint, status, event}, responder) {
    let ret = null;
    let response = null;
    if (trashpoint) {
        ret = await db.getTrashpointImages(trashpoint.id, status);
        response = ret.map(img => util.object.filter(img, {
            id: true, trashpointId: true, type: true, file: true, parentId: true,
            server: true, url: true,
        }));
    }
    if (event) {
        ret = await db.getEventImages(event.id, status);
        response = ret.map(img => util.object.filter(img, {
            id: true, eventId: true, type: true, file: true, parentId: true,
            server: true, url: true,
        }));
    }
...
module.exports = function () {
    ...
    lucius.register('role:db,cmd:getEventImages', async function (connector, args, __) {
        //args: eventId
        return connector.input(args)
        // verify that the event exists
            .request('role:db,cmd:getEventById', {id: args.eventId})
            // fetch images from db
            .input(event => ({
                trashpoint: null,
                status: Image.STATUS_READY,
            ...
            .use(async function ({images, blobs}, responder) {
                const response = images.map(img => {
                    const ret = util.object.filter(img, {
                        id: true, eventId: true, type: true, parentId: true,
                    });
                    ret.url = img.url ? img.url : (blobs.find(item => item.imageId === img.id)).url;
                    return ret;
                });
    ...
    lucius.register('role:db,cmd:allocateEventImages', async function (connector, args, __) {
        // args: eventId, request{count}
        return connector.input(args)
        // verify that the event exists
            .request('role:db,cmd:getEventById', {id: args.eventId})
            // create image entries in db
    ...
    lucius.register('role:db,cmd:confirmEventImages', async function (connector, args, __) {
        // args: eventId, request{confirmed[string], failed[string]}
        return connector.input(args)
        // verify that the event exists
            .request('role:db,cmd:getEventById', {id: args.eventId})
            // update image entries in db
            .use(async function (event, responder) {
                ...
                // update event ts
                if (anythingChanged) {
                    await db.touchEvent(event.id, __.user.id);
                }
                // compose response
                if (errors.length) {
                    return responder.failure(errors);
    ...
    lucius.register('role:db,cmd:deleteEventImages', async function (connector, args, __) {
        // args: eventId, request.delete[image id, image id, ...]
        return connector.input(args)
        // verify that the event exists
            .request('role:db,cmd:getEventById', {id: args.eventId})
            // delete images
            .use(async function (event, responder) {
                ...
                for (let i = 0; i < images.length; i++) {
                    ...
                    // check image event
                    else if (mainImg.eventId !== event.id) {
                        errors.push(new LuciusError(E.IMAGE_NOT_IN_TRASHPOINT,
                            {id: mainImageId, eventId: event.id}));
                        continue;
                    }
                    // all checks passed, process dependent images
                    let deletedAllDeps = true;
                    // find the dependents
                    const dependents = await db.getChildImages(mainImageId, null, event.id);
                    for (let k = 0; k < dependents.length; k++) {
                        const depImgId = dependents[k].id;
